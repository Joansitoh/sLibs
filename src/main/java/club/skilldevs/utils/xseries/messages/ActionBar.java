package club.skilldevs.utils.xseries.messages;

/**
 * Class for project sLibs
 * Date: 22/02/2022 - 18:24.
 *
 * @author Joansiitoh
 */
import club.skilldevs.utils.CooldownManager;
...
public final class ActionBar {
    ...
    public static void sendActionBar(@Nonnull Player player, @Nullable String message, int delay) {
        Objects.requireNonNull(player, "Cannot send action bar to null player");
        if (CooldownManager.hasCooldown(player.getUniqueId(), "libs-actionbar")) return;
        if (delay > 0) CooldownManager.addCooldown(player.getUniqueId(), "libs-actionbar", delay);
     ...
     * @param message the message to send.
     *
     * @see #sendActionBar(Player, String, int)
     * @since 1.0.0
     */
    public static void sendPlayersActionBar(@Nullable String message) {
        for (Player player : Bukkit.getOnlinePlayers()) sendActionBar(player, message, 0);
    }
    ...
    public static void clearActionBar(@Nonnull Player player) {
        CooldownManager.removeCooldown(player.getUniqueId(), "libs-actionbar");
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try {
                    ...
                    sendActionBar(player, message.call(), 0);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
